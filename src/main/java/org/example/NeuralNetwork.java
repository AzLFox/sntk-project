    ...
    public NeuralNetwork(double learningRate, UnaryOperator<Double> activation, UnaryOperator<Double> derivative, int... sizes) {
        ...
        for (int i = 0; i < sizes.length; i++) {
            ...
            for (int j = 0; j < sizes[i]; j++) {
                ...
                System.out.println("Нейрон " + i + "-ого слоя, "+ j + "-ый нейрон с начальным значением " + layers[i].biases[j]);
                for (int k = 0; k < nextSize; k++) {
                    layers[i].weights[j][k] = Math.random() * 2.0 - 1.0;
                    System.out.println("Вес соединения от " + j + "-ого нейрона, к " + k + "-ому нейрону следующего слоя");
                }
            }
        }
    }
